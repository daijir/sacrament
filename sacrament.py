    ...
    with open(MEMBERS_CSV, mode="r", encoding="utf-8-sig") as f:
        ...
        for row in reader:
            m = Member(
                ...
                is_bishopric=row["is_bishopric"].strip().lower() == "true",
                couple_talk_together=row["couple_talk_together"].strip().lower() == "true",
                has_small_children=row["has_small_children"].strip().lower() == "true",
                is_new_member=row["is_new_member"].strip().lower() == "true",
            )
            members[m.id] = m
...
def cmd_plan(args):
    ...
    for _ in range(args.weeks):
        ...
        is_fast = (current.day <= 7)
        assigned_today = []

        inv_str = "-"
        ben_str = "-"
        yt_str = "【証し会】" if is_fast else "-"
        at1_str = "-"
        at2_str = "-"
        ...
        m_pool.sort(key=lambda m: calculate_score(m, current, "prayer", assigned_today)[0], reverse=True)
        f_pool.sort(key=lambda m: calculate_score(m, current, "prayer", assigned_today)[0], reverse=True)

        inv = m_pool[0]
        ben = f_pool[0]
        inv_str = f"{inv.name}"
        ben_str = f"{ben.name}"
        assigned_today.extend([inv, ben])
        ...
        if not is_fast:
            ...
            pool2.sort(key=lambda m: calculate_score(m, current, "talk", assigned_today)[0], reverse=True)
            sp2 = pool2[0]
